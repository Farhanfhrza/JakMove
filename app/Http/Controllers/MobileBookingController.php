...
class MobileBookingController extends Controller
{
    ...
    {
        $ticket = Ticket::with(['transportType', 'pickupStop', 'dropoffStop'])
            ->where('user_id', auth()->id()) // opsional: hanya ambil tiket milik user yang login
            ->findOrFail($id);

        return view('mobile.ticket-detail', compact('ticket'));
    }

    public function chooseTransport()
    {
        $types = TransportType::all();
        return view('mobile.choose-transport', compact('types'));
    }
    ...
    public function chooseStops($transportType)
    {
        ...
        return view('mobile.choose-stops', compact('type', 'stops', 'prices'));
    }
    ...
    public function payTicket(Request $request)
    {
        ...
        return redirect()->route('mobile.dashboard')->with('success', 'Tiket berhasil dipesan!');
    }

    public function dashboard()
    {
        $tickets = auth()->user()->tickets;
        return view('mobile.dashboard', compact('tickets'));
    }
}
